...
#             new_k = k.replace('image_encoder', 'backbone.backbone')
#             # if 'attn' in new_k:
    ...
    for k, v in ckpt.items():
        if k.startswith('image_encoder'):
            ...
            if 'attn.qkv' in new_k:
                new_k = new_k.replace('attn.qkv', 'attn.qkv.qkv')
            # elif 'mlp' in new_k:
            #     new_k = new_k.replace('mlp', 'mlp.mlp')
        elif k.startswith('prompt_encoder'):
            new_k = k.replace('prompt_encoder', 'neck')
...
def main():
    ...
    #
    # state_dict = torch.load('/mnt/disk/mxf/projects/focsam-master/pretrain/sam_pretrain_vit_base.pth',
    #                         map_location='cpu')
    # print(state_dict['model'].keys(),'sgaikfiwefjioluwoiasjfoiaioesjgfoasfiofuoqwjofla\n',weight.keys())
    # torch.save(convert_vit( weight ,state_dict['model']), '/mnt/disk/mxf/projects/focsam-master/pretrain/sam_pretrain_vit_base_LoRA.pth')
