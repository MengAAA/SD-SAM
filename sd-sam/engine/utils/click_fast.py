...
def fast_generate_clicks(pre_labels: torch.Tensor,
    ...
    # print('click fast 里面 point lists   ' , points_list)
    # # 在 fast_generate_single_click 函数中添加可视化代码
    # fneg = (~pre_labels) & seg_labels
    # fpos = pre_labels & (~seg_labels)
    # for i in range(pre_labels.shape[0]):
    #     plt.figure(figsize=(10, 5))
    #     plt.subplot(1, 2, 1)
    #     plt.imshow(fneg[i].squeeze().cpu().numpy())
    #     plt.title("False Negative")
    #     plt.subplot(1, 2, 2)
    #     plt.imshow(fpos[i].squeeze().cpu().numpy())
    ...
    for pre_label, seg_label, points in zip(
        ...
        clicks.append(fast_generate_single_click(
            ...
            **kwargs))
    # print('click fast 里面 clicks    ', clicks)
