...
'''文件主要实现了用于图像分割任务中生成交互点击点的功能。在交互式图像分割里，用户可以通过点击图像上的点来辅助模型更精准地分割目标物体，此文件的代码能够依据预测标签和真实标签自动生成这些点击点。'''
...
'''常量定义：
CLK_POSITIVE 和 CLK_NEGATIVE：分别代表正点击和负点击的模式。
CLK_POSITIVE_ID 和 CLK_NEGATIVE_ID：分别对应正点击和负点击的 ID。
CLK_MODES 和 CLK_MODE_IDS：包含正、负点击模式及其 ID 的元组。'''
...
def generate_multiple_clicks(num_clicks: int,
    ...
    """
    # Check types and shapes of input labels   检查输入标签的类型、形状以及已有点击点的有效性
    ...
    # Calculate the distance scale based on sfc_inner_k   依据 sfc_inner_k 计算距离缩放因子
    if sfc_inner_k >= 1.0:
    ...
    # Convert labels to numpy arrays  将标签转换为 numpy 数组
    ...
    # Create ignore mask based on points   创建忽略掩码，排除已有点击点
    ignore_mask = np.zeros_like(pre_label, dtype=bool)
    for y, x, _ in (list() if points is None else points):
        ignore_mask[y, x] = True

    # Calculate erroneous regions and perform distance transform   计算预测错误的区域（假负和假正），并进行距离变换
    ...
    # Calculate maximum distances    比较假负和假正区域的最大距离，确定点击模式
    ndmax, pdmax = ndist.max(), pdist.max()
    if ndmax == pdmax == 0:
        return [(None, None, None) for _ in range(num_clicks)]

    # Determine click mode and points based on maximum distances    选取满足距离条件的点，随机打乱后选取前 num_clicks 个点作为点击点
    if ndmax > pdmax:
        mode = CLK_POSITIVE
...
def generate_single_click(pre_label: Union[torch.Tensor, np.ndarray],
    ...
    """
    功能：生成单个点击点。
    实现步骤：和 generate_multiple_clicks 函数基本相同，只是最后随机选择一个点作为点击点。
    ...
    fpos = np.logical_and(fpos, ~ignore_mask)
    if dist_mode == 'plain':
        ndist = mask_to_distance(fneg, True)     #  mask_to_distance
        pdist = mask_to_distance(fpos, True)
    elif dist_mode == 'fast':
        ndist = fast_mask_to_distance(fneg, True)
        pdist = fast_mask_to_distance(fpos, True)
...
def generate_clicks(pre_labels: Union[torch.Tensor, np.ndarray],
    ...
    for pre_label, seg_label, points in zip(
        ...
        clicks.append(generate_single_click(
            ...
            **kwargs))
    return clicks
    # 功能：为一组预测标签和真实标签生成单个点击点。
    # 参数：
    # pre_labels：一组预测标签，形状为 (batch_size, height, width)。
    # seg_labels：一组真实标签，形状为 (batch_size, height, width)。
    # points_list：每组标签对应的已有点击点列表。
    # sfc_inner_k：点击区域的调整因子。
    # 实现步骤：遍历每组标签，调用 generate_single_click 函数生成单个点击点。
