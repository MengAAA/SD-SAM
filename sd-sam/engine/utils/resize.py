...
def resize_along_longest_side(hw_shape, target_length):   # 根据目标长度 target_length 计算输入形状 hw_shape 沿着最长边缩放后的新高度和新宽度
    scale = float(target_length) / max(hw_shape)
    return int(scale * hw_shape[0] + 0.5), \
           int(scale * hw_shape[1] + 0.5)


def resize_image_along_longest_size(image, target_length, **kwargs):   # 将输入的图像 image 沿着最长边缩放到目标长度 target_length
...
def resize_coord_along_longest_size(coord, hw_shape, target_length):   # 将输入的坐标 coord 随着图像的缩放而进行相应的缩放
...
def resize_binary_mask_by_polygons(mask, target_length, ignore_index=255):   # 将输入的二值掩码 mask 沿着最长边缩放到目标长度 target_length
