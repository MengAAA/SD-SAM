...
class LoadNEUAnnotations(LoadAnnotations):

    def _load_seg_map(self, results):

        # print(results)
        # print("results keys in LoadNEUAnnotations:", results.keys())  # 添加打印语句
        super(LoadNEUAnnotations, self)._load_seg_map(results)
        # print('abcdefsfadas')
...
class NEUDataset(BaseInterSegDataset):

    METAINFO = dict(
        classes = ('good', 'defect'),
        palette = [[0, 0, 0], [255, 0, 0]]
    )
    default_meta_root = 'data/meta-info/neu_mvtec.json'
